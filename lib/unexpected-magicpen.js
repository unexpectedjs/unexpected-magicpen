  ...
  installInto: function unexpectedMagicPen(expect) {
    ...
    expect.addStyle('magicPenLine', function (line, pen) {
      line.forEach(function (lineEntry, j) {
        ...
        if (lineEntry.style === 'text') {
          const styles = lineEntry.args.styles;
          if (
            pen &&
            styles.length === 1 &&
    ...
    expect.addStyle('magicPen', function (pen, lines) {
      ...
      this.block(function () {
        ...
        if (!pen.isEmpty()) {
          const inspectOnMultipleLines = lines.length > 1 || lines[0].length > 1;
          if (inspectOnMultipleLines) {
            this.nl().indentLines().i();
          }
          this.block(function () {
      ...
      if (
        isTopLevel &&
        !pen.isEmpty() &&
        (pen.format === this.format || !pen.format)
      ) {
    ...
    expect.addAssertion(
      '<Error> to have (ansi|html|text|) (message|diff) <any>',
      function (expect, subject, value) {
        expect.errorMode = 'nested';
        const format = expect.alternations[0] || 'text';
        const useDiff = expect.alternations[1] === 'diff';
        if (subject.isUnexpected) {
          let subjectPen;
          if (useDiff) {
            const diff = subject.getDiff({ format });
            if (diff) {
              subjectPen = diff;
            } else {
            ...
            subjectPen = subject.getErrorMessage({ format });
          }
          ...
          if (valueType.is('magicpen')) {
            ...
            const expectedOutput = expect.createOutput(format);
            const returnValue = value.call(expectedOutput, subjectPen.toString());
          ...
          if (format !== 'text') {
            expect.fail(
              'Cannot get the ' +
                format +
                ' representation of non-Unexpected error'
            );
          } else {
